...
const webeSidebarShell = document.querySelector('.sidebar-shell');
...
export const currentDOM = async () => {
	if (store.state.checkHead === true) {
		...
		const checkSidebar = async () => {
			webeSidebarShell.addEventListener('DOMContentLoaded', event => {
				event.preventDefault();

				if (event.composedPath().includes(this)) {
					store.dispatch('webeSidebarShell', event);
				}
			});
			await new Promise(
				resolve => setTimeout(resolve, 100),
